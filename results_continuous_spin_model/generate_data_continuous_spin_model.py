...
sys.path.insert(0, '..')
import continuous_spin_model as spin_model


def save_data(file_name, J, S, S1, beta, args):
    """
    Save model data for continuous spin model.

    Arguments:
        file_name (str): Path to the output .npz file.
        J : np.ndarray
            Coupling matrix parameters.
        S : 2d np.array (float32)
            Angles in radians at time t.
        S1 : 2d np.ndarray (float32)
            Angles in radians at time t+1 (single sweep later).
    """
    np.savez(
        file_name,
        J=J,
        S=S,      # raw angles
        S1=S1,    # updated angles
        beta=beta, 
        N=args.N,
        DJ=args.DJ,
        J0=args.J0,
...
BASE_DIR_MODE = BASE_DIR + '/' + ("cont_sequential" if args.sequential else "cont_parallel")
...
for beta_ix, beta in enumerate(betas):
    ...
    print(f"\n# ** Running simulation {beta_ix+1}/{len(betas)} for N={args.N} rep={args.rep} β={beta} **", flush=True)
    ...
    S, S1 = spin_model.run_simulation(
        beta=beta, 
        J=J, 
    ...
    print(f'Sampled {S.shape[0]} states in  {time.time()-start_time:.3f}s')
    save_data(file_name, J, S, S1, beta, args)
