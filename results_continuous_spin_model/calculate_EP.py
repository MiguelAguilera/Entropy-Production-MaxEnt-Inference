    ...
    for ib, beta in enumerate(np.round(betas, 8)):
        ...
        if args.patterns is None:
            if args.num_neighbors is None:
                file_name = f"{BASE_DIR}/cont_sequential/run_reps_{rep}_N_{N:06d}_beta_{beta}_J0_{args.J0}_DJ_{args.DJ}.npz"
                file_name_out = f"{SAVE_DATA_DIR}/results_N_{N}_reps_{rep}_beta_{beta}_J0_{args.J0}_DJ_{args.DJ}.h5"
            else:
                file_name = f"{BASE_DIR}/cont_sequential/run_reps_{rep}_N_{N:06d}_beta_{beta}_J0_{args.J0}_DJ_{args.DJ}_num_neighbors_{args.num_neighbors}.npz"
