...
sys.path.insert(0, '..')
import ep_multipartite as ep_estimators
import utils
...
def calc_spin(i_args):
    ...
    num_chunks=5
#    num_chunks=-1
    est = ep_estimators.EPEstimators(S_i_t, i, num_chunks=num_chunks)

    # Calculate empirical estimate of true EP
    spin_emp  = 0# beta * (J_without_i @ data.g_mean).item()
...
#    gc.collect()
    ...
    t0 = time.time()
    sig_Gaussian, theta_Gaussian, _ = est.get_EP_Newton(max_iter=1, holdout=False, adjust_radius=True)
    N1 = Pi * sig_Gaussian
...
#    gc.collect()
    ...
    sig_MaxEnt, theta_MaxEnt, _ = est.get_EP_Newton(trust_radius=0.25, holdout=False, adjust_radius=False)
    N2 = Pi * sig_MaxEnt
